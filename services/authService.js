...
exports.signup = asyncHandler(async (req, res, next) => {
  try {
    ...
    const createdUser = new User({
      ...
      email,
      password,
      role: role === 'admin' ? 'user' : role, // Prevent direct admin creation via role field, forcing overrides below if needed, or just let valid roles through. 
      // Actually, the original logic allowed role='admin' to set role='admin' AND isAdmin=true. 
      // Let's stick to cleaning req.body.
      role
    });
    if (role === 'admin') {
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      res.status(404).json({ message: `There is no user with email ${req.body.email}` })
    );
  }
